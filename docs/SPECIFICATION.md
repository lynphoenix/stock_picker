...
#### 2. 自然语言策略回测 (Natural Language Strategy Backtest)

**功能描述:**
用户用自然语言描述交易策略，系统自动解析策略参数，筛选符合条件的股票，运行回测并返回结果。

**完整工作流程:**
```
用户输入策略 → AI解析策略参数 → 筛选股票池 → 运行回测 → 展示结果
     ↓              ↓                ↓           ↓          ↓
"均线多头     提取:            AI板块      2013-2024   收益率
 ROE>15%"      MA(5,20)         3000只       全市场      +25.5%
               ROE>15%           筛选后      模拟交易     胜率65%
                                  50只        风控应用     最大回撤-12%
```
...
```
用户: "我要测试一个策略：均线多头排列，ROE大于15%，
      市值大于50亿的AI板块股票，止损-10%，止盈+20%"
...
|------|----------|----------|
| **技术指标** | "均线多头" | MA5 > MA20 |
|  | "MACD金叉" | MACD.DIF > MACD.DEA |
|  | "RSI超卖" | RSI < 30 |
|  | "布林带突破" | Price > BOLL.upper |
| **基本面** | "ROE大于15%" | ROE > 15 |
|  | "营收增长大于20%" | Revenue_growth > 20 |
|  | "市盈率小于30" | PE < 30 |
| **筛选条件** | "AI板块" | sector = "人工智能" |
|  | "市值大于50亿" | market_cap > 50亿 |
|  | "剔除ST" | exclude ST stocks |
| **风控规则** | "止损10%" | stop_loss = -10% |
|  | "止盈20%" | take_profit = +20% |
|  | "最大持仓5只" | max_positions = 5 |
|  | "单只不超过20%" | max_position_size = 20% |

**测试文件:**
- 策略解析测试: `backend/tests/__tests__/strategy/test_parser.test.py`
- 回测API测试: `backend/tests/__tests__/backtest/test_backtest_api.test.py`
- E2E测试: `frontend/e2e/strategy-backtest.spec.ts`

**API端点:**
- `POST /api/strategy/parse` - 解析自然语言策略
- `POST /api/strategy/backtest` - 运行策略回测
- `GET /api/strategy/results/{id}` - 获取回测结果
